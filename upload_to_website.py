...
if stenodicts_website_path.exists():
    ...
    for md_file_string in glob.iglob(repo_path + "/*/*.md", recursive=True):
        ...
        with codecs.open(
            str(stenodicts_website_path / md_file_path.name), "w", "utf-8"
        ) as output_file:
            output_file.write("---\n")
    ...
    os.system(
        "cd "
        + repo_path
        + "/../stenodicts-website && JEKYLL_ENV=production bundle exec jekyll build"
    )
    ...
    os.system(
        "rsync -avh "
        + repo_path
        + "/../stenodicts-website/_site/ webspace:/www/htdocs/\\*/mkrnr.com/stenodicts --delete"
    )
